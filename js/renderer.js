/* RPGAtlas — renderer.js
   HD-2D renderer. Raw WebGL2, no dependencies, no build step.
   The map is a 3D scene viewed through a tilted perspective camera: tiles with
   a height extrude into blocks (top face textured from the map prerender,
   exposed sides auto-shaded), characters are upright billboards standing at
   their tile's elevation, and overhead tiles hover one tile unit above their
   ground. World units are pixels: X = map x, Z = map y (south, toward the
   camera), Y = up = height * TILE.
   Atmosphere, each individually toggleable per map via map.hd2d:
     tilt   — camera pitch in degrees (25 = low/leaning back .. 89 = top-down)
   ...
   Originally Phase-3 gl.js; the PIXI v8 detour (2026) is gone — this is the
   true-3D path the camera tilt needs. The pure height-extrusion helpers
   (planWalls / planLightOccluders) stay exported for the unit tests.
   Copyright (C) 2026 RPGAtlas contributors — GPL-3.0-or-later (see LICENSE). */
"use strict";

const Renderer = (() => {
  // Resolved lazily-safe: this file is loaded in a bare VM by the wall-extrusion
  // unit test (no window.Assets), so fall back to the engine's 48px tile.
  ...
  // ============================ pure geometry ============================
  // Exposed for unit tests (tests/wall-extrusion.test.js) and load-safe: these
  // touch nothing but TILE and Math, so the file imports without a GPU or DOM.

  // One entry per elevated tile. `faceUnits` is how far the south wall is
  // exposed over its southern neighbour (units of height) — the only face a
  // fixed top-down summary needs. The 3D path below does its own full extrusion.
  ...
  // context is ever lost we permanently report false and the host falls back to
  // the Canvas 2D path (all GPU resources are invalid after a loss). Async to
  // match the host's `await Renderer.available()` call sites.
  //
  // options.canvas — render into this existing canvas (the editor's HD-2D
  //   preview panel). When omitted, a canvas is created and inserted behind the
  //   game's #gamecanvas (which the engine leaves transparent over the map).
  ...
  // ---------------------------- render targets ----------------------------
  let rt = null; // { w, h, scene:{fbo,tex}, depthTex, half:[{fbo,tex} x4], hw, hh }
  ...
  // extra (optional): { zoom, shakeX, shakeY, tilt, ambient,
  //                     focus: {rx, ry} — player, for depth of field;
  //                     lights: [{rx, ry, color, radius}] — active light events }
  function renderFrame(w, h, camX, camY, sprites, extra) {
    if (!ok || !gl || gl.isContextLost()) return null;
    extra = extra || {};
    if (cv.width !== w || cv.height !== h) { cv.width = w; cv.height = h; }

    const tiltDeg = Math.min(89, Math.max(25,
      extra.tilt != null ? Number(extra.tilt) : cfg.tilt));
    const pitch = tiltDeg * Math.PI / 180;
    const zoom = Math.max(0.25, Math.min(4, Number(extra.zoom) || 1));
    const ambient = extra.ambient != null
      ? Math.min(2, Math.max(0, Number(extra.ambient)))
      : cfg.ambient;
    ...
    const shX = (extra.shakeX || 0) / zoom, shZ = (extra.shakeY || 0) / zoom;
    const tX = camX + w / zoom / 2 + shX, tZ = camY + h / zoom / 2 + shZ;
    ...
    gl.enable(gl.BLEND);
    // The GL canvas is the bottom layer (the engine's 2D #gamecanvas sits on top,
    // transparent over the map), so clear opaque — this canvas paints the scene.
    ...
    gl.uniform1f(sceneProg.u.uAmbient, ambient);
