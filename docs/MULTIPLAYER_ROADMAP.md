...
**Status:** ROADMAP APPROVED 2026-07-19 · **Phase MP4 COMPLETE** (Fable gate PASS 2026-07-19, tag `beacon-4`; MP3 `beacon-3`; MP2 `beacon-2`; MP1 `beacon-1`; MP0 `beacon-0`) · next: MP5 · Tags: `beacon-0` … `beacon-9` + `v2.0.0`
...
| MP5 | Beacon server, rooms & transport | Opus | **Fable security gate** | beacon-5 | 🚧 BUILD COMPLETE 2026-07-19 (Opus, stages A–E) — awaiting Fable SECURITY gate. Open-source Beacon server ships: one TS core (server/src/core), two targets (Node `ws` + Cloudflare DO, one room/DO, hibernation), server-authoritative friend rooms end-to-end. Server sim = player layer + **static wall collision** (Driftwood branch answer) via headless `src/shared/sim/collision.ts` (mirrors engine `tilePassable`, CI drift-guarded vs js/assets.js); NPC/events/persistence → MP8 (D-5-0), one configured game (D-5-1). Client `socket-transport` + `RelayClient` + gated "Play Together" title flow (create/join, friendly errors) — title byte-identical when MP off → goldens safe. Hardening: strict decode, byte cap, message + join rate limits, resume-token rotation, empty-room expiry, fuzz suite (5000 garbage frames). Gates: vitest 1109 + net 7 (=1116, +41) · node 46 · Playwright 126/126 (goldens byte-identical, `mp-relay` real-server+native-WS green) · root+server(Node+CF) tsc 0 · eslint 0 · 16-bot+2 latency **p95 ≈ 32 ms** (budget 150) · v1.2.0 · FV 2 · no cache-busts. Details: docs/mp-5-spec.md. |
